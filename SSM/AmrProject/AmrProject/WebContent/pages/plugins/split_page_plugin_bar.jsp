...
<div id="splitBarDiv" style="float: right">
	<ul class="pagination pagination-sm">
		...
		%>
		<li><a
			href="<%=url%>?cp=<%=currentPage - 1%>&col=<%=column%>&kw=<%=keyWord%>&<%=paramName%>=<%=paramValue%>&<%=paramNameB%>=<%=paramValueB%>">上一页</a></li>
		<li><a
			href="<%=url%>?cp=1&col=<%=column%>&kw=<%=keyWord%>&<%=paramName%>=<%=paramValue%>&<%=paramNameB%>=<%=paramValueB%>">1</a></li>
		<%
			}
		%>
		...
		<li class="<%=currentPage == x ? "active" : ""%>"><a
			href="<%=url%>?cp=<%=x%>&col=<%=column%>&kw=<%=keyWord%>&<%=paramName%>=<%=paramValue%>&<%=paramNameB%>=<%=paramValueB%>"><%=x%></a></li>
		<%
			}
			} else { // 考虑到省略号的问题，6页以后
		...
		%>
		<li><a
			href="<%=url%>?cp=<%=pageSize%>&col=<%=column%>&kw=<%=keyWord%>&<%=paramName%>=<%=paramValue%>&<%=paramNameB%>=<%=paramValueB%>"><%=pageSize%></a></li>
		<li><a
			href="<%=url%>?cp=<%=currentPage + 1%>&col=<%=column%>&kw=<%=keyWord%>&<%=paramName%>=<%=paramValue%>&<%=paramNameB%>=<%=paramValueB%>">下一页</a></li>
		<%
			}
		%>
		</li>
