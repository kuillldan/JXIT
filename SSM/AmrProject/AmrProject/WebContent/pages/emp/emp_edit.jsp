							...
							<form class="form-horizontal" action="<%=editUrl%>" id="myform" method="post">
								<fieldset>
									...
									<div class="form-group" id="sexDiv">
										...
										<div class="col-md-5">
											<div class="radio-inline">
												<label><input type="radio" name="sex" id="sex" value="男" ${vo.sex=="男"?"checked":"" }>男</label>
											</div>
											<div class="radio-inline">
												<label><input type="radio" name="sex" id="sex" value="女" ${vo.sex=="女"?"checked":"" }>女</label>
											</div>
										</div>
										<!-- 定义表单错误提示显示元素 -->
										...
										<div class="col-md-5">
											<select id="dept.did" name="dept.did" class="form-control">
												<c:forEach items="${allDepts }" var="dept">
													<option value="${dept.did }" ${vo.dept.did==dept.did?"selected":"" }>人事部</option>
												</c:forEach>
											</select>
										</div>
										<!-- 定义表单错误提示显示元素 -->
										...
										<div class="col-md-5">
											<select id="level.lid" name="level.lid" class="form-control">
												<c:forEach items="${allLevels }" var="level">
													<option value="${level.lid }" ${level.lid==vo.level.lid?"selected":"" }>${level.title }&nbsp;${level.losal }-${level.hisal }元</option>
												</c:forEach>
											</select>
										</div>
										<!-- 定义表单错误提示显示元素 -->
									...
									<div class="form-group" id="notice.noteDiv">
										...
										<div class="col-md-5">
											<!-- 定义表单输入组件 -->
											<textarea id="note" name="note" class="form-control" placeholder="请输入公告信息" rows="10">${vo.note }</textarea>
										</div>
										<!-- 定义表单错误提示显示元素 -->
