  	...
  	<form action="<%=goodsUpdateURL%>" method="post" enctype="multipart/form-data">
  		<table border="1" cellpadding="5" cellspacing="0" bgcolor="F2F2F2" width="80%">
  			...
  			<tr>
  				...
  				<td width="55%" rowspan="7"><div id="previewDiv">
  			...
  			<tr>
  				<td>发布状态</td>
  				<td>
  					<input type="radio" name="status" ${goods.status==0?"checked":"" } value="0" >下架
  					<input type="radio" name="status" ${goods.status==1?"checked":"" } value="1">上架
  					<input type="radio" name="status" ${goods.status==2?"checked":"" } value="2">删除
  				</td>
  			</tr>
  			<tr>
  				<td>商品描述</td>
  				<td>
  					<textarea rows="5" cols="60" id="note" name="note" >${goods.note }</textarea>
  				</td>
  			</tr>
  			<tr>
  				<td colspan="3">
  		...
  		<input type="hidden" id="bow" name="bow" value="${goods.bow }">
