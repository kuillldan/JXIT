...
public class ProxyServlet extends HttpServlet
{
	...
	protected static final String P_TARGET_URI = "redirectURL";
	protected static final String P_PATH_TRIM = "PathTrim";
	protected static final String P_PATH_PREPEND = "PathPrepend";
	...
	protected HttpClient createHttpClient(HttpParams hcParams)
	{
		...
		String s = System.getProperty("http.maxConnections", "100");
		final int max = Integer.parseInt(s);
		manager.setDefaultMaxPerRoute(max);
		manager.setMaxTotal(2 * max);
		DefaultHttpClient client = new DefaultHttpClient(manager, hcParams);
		return client;
	}
	...
	protected Boolean doPathTrim(StringBuilder sb, String path, String trimStr)
	{
		...
		{
			String lTrimPath = trimStr;
			if (!lTrimPath.startsWith("/"))
			{
				lTrimPath = "/" + lTrimPath;
			}
			if (lTrimPath.endsWith("/"))
			{
				lTrimPath = lTrimPath.substring(0, lTrimPath.length() - 1);
			}

			if (path.startsWith(lTrimPath + "/"))
			{
				sb.append(path.substring(lTrimPath.length()));
				res = Boolean.TRUE;
			} else if (path.equals(lTrimPath))
			{
				sb.append("");
				res = Boolean.TRUE;
			} else
	...
	protected String rewriteRequestPath(HttpServletRequest servletRequest)
	{
	...
	protected String rewriteUrlFromResponse(HttpServletRequest servletRequest, String theUrl)
	{
		...
		if (theUrl.startsWith(targetUri))
		{
			...
			String rediPath = theUrl.substring(targetUri.length());

			StringBuilder sb = new StringBuilder();
			if (Boolean.TRUE.equals(servletRequest.getAttribute(ATTR_PATH_PREPEND)))
			{
				if (Boolean.TRUE.equals(doPathTrim(sb, rediPath, pathPrepend)))
				{
					if (Boolean.TRUE.equals(servletRequest.getAttribute(ATTR_PATH_TRIM)))
					{
						doPathPrepend(sb, pathTrim);
					}
				}
			} else
			{
