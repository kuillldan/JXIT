...
<body>
	<div class="p">
		<%
			...
			//  }
			if (pages == pagescount && count % pagesize != 0)
			{//感谢shixiangkun在博客中提出问题
			...
			if (pages > 1)
			{
				response.getWriter().println("<a href=?pages=" + (pages - 1) + ">上一页</a>");
			}//>显示上一页
				//<显示分页码
			for (int i = listbegin; i < listend; i++)
			{
				if (i != pages)
				{//如果i不等于当前页
					response.getWriter().println("<a href=?pages=" + i + ">[" + i + "]</a>");
				} else
				{
			...
			if (pages != pagescount)
			{
				response.getWriter().println("<a href=?pages=" + (pages + 1) + ">下一页</a>");
			}//>显示下一页
				//>显示分页信息
		%>
	</div>
